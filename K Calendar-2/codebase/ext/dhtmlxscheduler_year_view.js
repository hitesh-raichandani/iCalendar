scheduler.config.year_x = 4, scheduler.config.year_y = 3, scheduler.xy.year_top = 0, scheduler.templates.year_date = function(e) {
    ...
    scheduler.dblclick_dhx_month_head = function(t) {
        if (e()) {
            ...
            if ( - 1 != r.parentNode.className.indexOf("dhx_before")||-1 != r.parentNode.className.indexOf("dhx_after"))
                return !1;
    ...
    scheduler.clear_view = function() {
        ...
        for (var s in t)
            t.hasOwnProperty(s) && (r = t[s], r.className = "dhx_month_head", r.setAttribute("date", ""));
        ...
        if (this._tooltip) {
            if (this._tooltip.date.valueOf() == e.valueOf())
                return;
        ...
        for (t = t.valueOf() < this._min_date.valueOf() ? this._min_date : this.date.date_part(new Date(t)); t < e.end_date;)
            if (this._mark_year_date(t, e), t = this.date.add(t, 1, "day"), t.valueOf() >= this._max_date.valueOf())
                return
        ...
        if (e && (t = scheduler.xy.scale_height, scheduler.xy.scale_height =- 1), scheduler._els.dhx_cal_header[0].style.display = e ? "none" : "", scheduler.set_sizes(), e && (scheduler.xy.scale_height = t), scheduler._table_view = e, !this._load_mode ||!this._load())
            if (e) {
                if (scheduler._init_year_tooltip(), scheduler._reset_year_scale(), scheduler._load_mode && scheduler._load())
                    return void(scheduler._render_wait=!0);
        ...
        for (var d = s - 11, n = 0, i = document.createElement("div"), l = this.date.week_start(scheduler._currentDate()), _ = 0; 7 > _; _++)
            this._cols[_] = Math.floor(d / (7 - _)), this._render_x_header(_, n, l, i), l = this.date.add(l, 1, "day"), d -= this._cols[_], n += this._cols[_];
        ...
        for (var o = this.date[this._mode + "_start"](this.date.copy(this._date)), c = o, h = null, _ = 0; _ < r.year_y; _++)
            for (var u = 0; u < r.year_x; u++) {
                ...
                for (var v = this.date.week_start(o), f = this._reset_month_scale(h.childNodes[2], o, v), p = h.childNodes[2].firstChild.rows, g = p.length; 6 > g; g++) {
                    ...
                    for (var m = 0, y = p[g].childNodes.length; y > m; m++)
                        p[g].childNodes[m].className = "dhx_after", p[g].childNodes[m].firstChild.innerHTML = scheduler.templates.month_day(f), f = scheduler.date.add(f, 1, "day")
                    }
    ...
    scheduler.getActionData = function(t) {
        if (!e())
            return n.apply(scheduler, arguments);
